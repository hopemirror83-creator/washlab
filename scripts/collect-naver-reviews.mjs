...
for (const [index, carwash] of carwashes.entries()) {
  ...
  if (old && !force && !(onlyEmpty && oldSourceCount === 0)) {
    results.push(old);
    continue;
  }
  ...
  console.log(`Naver blog search ${index + 1}/${carwashes.length}: ${carwash.name}`);
  ...
  collected += 1;
  await sleep(Number(process.env.NAVER_DELAY_MS || 160));
}

for (const old of existing) {
  if (!results.some((item) => item.sourceId === old.sourceId)) results.push(old);
}

await mkdir(DATA_DIR, { recursive: true });
await writeFile(OUTPUT_FILE, `${JSON.stringify(results, null, 2)}\n`, 'utf8');
console.log(`Wrote ${path.relative(ROOT, OUTPUT_FILE)}`);
console.log(`Collected groups: ${results.length}`);
console.log(`Refreshed groups: ${collected}`);

async function collectSources(carwash, queries) {
  const seen = new Set();
  const sources = [];
  for (const query of queries.slice(0, Number(process.env.NAVER_QUERY_MAX || 28))) {
    const items = await searchNaverBlog(query);
    await sleep(Number(process.env.NAVER_QUERY_DELAY_MS || 100));
...
function buildQueries(carwash) {
  ...
  const searchSignals = (carwash.querySignals || []).slice(0, 4);
...
async function searchNaverBlog(query) {
  ...
  url.searchParams.set('display', process.env.NAVER_DISPLAY || '30');
  url.searchParams.set('start', '1');
  url.searchParams.set('sort', 'sim');

  let response;
  for (let attempt = 1; attempt <= Number(process.env.NAVER_RETRY_COUNT || 4); attempt += 1) {
    ...
    const waitMs = Number(process.env.NAVER_RATE_WAIT_MS || 2500) * attempt;
...
function looksRelevant(source, carwash) {
  ...
  return score >= Number(process.env.NAVER_RELEVANCE_SCORE || 55);
}
...
function inferPlaceTokens(carwash) {
  const values = [
    ...
    ...String(carwash.roadAddress || carwash.lotAddress || '').match(/[가-힣0-9]+(?:동|읍|면|리|역|IC|ic|로|길)/g) || [],
  ];
...
function stripHtml(value) {
  return clean(value).replace(/<[^>]*>/g, '').replace(/&quot;/g, '"').replace(/&amp;/g, '&').trim();
}
